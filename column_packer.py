...
def populate_column(column):
    ...
    for drid in column.drids:
        ...
        available_roles = [role for role, count in zip(cell_roles, cell_counts) for _ in range(count)]
...
def establish_connections(column):
    ...
    all_cells = [(drid_idx, cell_key, drid.cell_dict[cell_key]) for drid_idx, drid in enumerate(column.drids) for cell_key in drid.cell_dict]
    ...
    for drid_idx, drid in enumerate(column.drids):
        for cell_key, cell_info in drid.cell_dict.items():
            ...
            input_cells = [cell for cell in all_cells if cell[2][-1] == (0, 1) or cell[2][-1] == (1, 1)]
            output_cells = [cell for cell in all_cells if cell[2][-1] == (1, 0) or cell[2][-1] == (1, 1)]
            ...
            drid.cell_dict[cell_key] = (cell_index, local_neighbors, role, code,input_non_local, output_non_local)

def create_random_column(io_number, ro_number, number_of_drids, grid_dimension, grid_size):
    ...
    for _ in range(number_of_drids):
        ...
        grid = Grid(grid_dimension, grid_size, decay_value, lower_tresh, upper_tresh, density, input_bias, output_bias, radio_bias)
